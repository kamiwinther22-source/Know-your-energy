...
- A placement that points toward wanting real partnership or closeness
  describes a genuine pull, not a behavior currently being practiced
  with someone. Never write it as an active pattern inside an existing
  relationship ("you keep circling back to," "your old pattern of
  going it alone") -- you don't know whether the person has a partner
  right now. Name the real want itself, and leave room for it to be an
  unmet desire, not just something being acted out.
- When a pattern costs something, name the specific real resource it
  spends -- physical energy, sleep, emotional steadiness, focus,
  patience with people -- instead of a vague "toll" or "cost."
...
- Personal Year/Month/Day, Essence (their cycle right now): assume the
  ...
  for them right now, not a footnote -- give it real space. Name which
  cycle you're describing and its real timeframe every time -- "this
  year," "this month," "today." The Personal Year and Essence are the
  real backdrop and hold for the reading's shelf life; the Personal Day
  is a single day, already different tomorrow, and must never be
  folded into that backdrop as if it carries the same weight -- name it
  separately, as today's texture. Mention the Personal Month only when
  it's doing real interpretive work, and say plainly that it's a
  shorter window than the year itself.
