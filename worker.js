...
function toCountryCode(country) {
  ...
  const map = {
    ...
    'mexico':'MX','cuba':'CU','jamaica':'JM','dominican republic':'DO',
...
async function getAstrology(env, dob, timeStr, city, state, country) {
  ...
  const res = await fetch('https://api.astrology-api.io/api/v3/data/positions', {
    ...
    body: JSON.stringify({
      subject: {
        birth_data: { year, month, day, hour, minute, city: cityStr, country_code }
      }
    }),
  });
  if (!res.ok) {
...
async function getHumanDesign(env, dob, timeStr, city, state, country) {
  const { year, month, day } = parseDOB(dob);
  const { hour, minute } = parseTime(timeStr);
  const cityStr = [city, state, country].filter(Boolean).join(', ') || 'New York, US';
  ...
  const res = await fetch('https://api.humandesignhub.app/v1/simple-bodygraph', {
    ...
    body: JSON.stringify({ date: dateStr, time: timeStrFmt, city: cityStr }),
  });
  if (!res.ok) {
    const errText = await res.text();
...
async function assemblePersonData(env, person) {
  ...
  const [astrology, humanDesign] = await Promise.all([
    ...
    getHumanDesign(env, dob, time, city, state, country),
  ]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Return raw chart data only — no Claude report yet
      return jsonResponse({
        p1: p1Data,
        p2: p2Data,
        report: {
