...
async function getHumanDesign(env, dob, timeStr, city) {
  ...
  // API requires a single "datetime" field in ISO 8601 format
...
async function assemblePersonData(env, person) {
  ...
  // Numerology only needs name + DOB — always runs
  let numerology = null;
  ...
  // Astrology runs whenever DOB is present; API handles missing time/location gracefully
  let astrology = null;
  let astrologyError = null;
  try {
    astrology = await getAstrology(env, dob, time, city, state, country);
  } catch (e) {
    astrologyError = e.message;
  }

  // Human Design requires time AND city for meaningful results — skip if either is missing
  ...
  if (hasTime && hasCity) {
    try {
      humanDesign = await getHumanDesign(env, dob, time, city);
    } catch (e) {
