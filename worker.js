...
9. State the real mechanism plainly, as the concrete fact it is. A metaphor whose only job is to sound profound ("several clocks running at different speeds") is a sign you've drifted from the fact -- return to stating it directly.
...
14. Don't manufacture a "tension" between two things that don't actually conflict. Before calling anything a pull in different directions, check: do these two facts really oppose each other, or do they just both happen to be true at once? Wanting to build something large and wanting quiet, gentle partnership are not opposites -- if two placements or numbers reinforce each other or simply coexist, say so plainly instead of forcing a struggle that isn't there.
