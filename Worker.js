...
// Combines separate date/time fields (as collected by the frontend) into
// the single ISO datetime string the Human Design API requires.
...
// Numerology is calculated locally (no vendor, no cost).
// Astrology and Human Design are external calls. Either can fail
// independently — a failure in one does not block the other, and
// the error is passed through explicitly rather than silently hidden,
// so Claude is only ever told about data that actually exists.
...
// Describes, honestly, which data sources actually succeeded for a person —
// so the prompt never asks Claude to synthesize a system that failed to load,
// and never lets Claude quietly guess at data that isn't really there.
