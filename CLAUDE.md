...
- Everything else requires `POST`:
  - `/check-pass` — reads a pass record from KV (`PASSES`); only ever
    returns `active: true` for the `UNLIMITED_EMAILS` family bypass. There
    is currently no payment processor wired up, so no other email can ever
    have an active pass.
...
- **Never write her copy.** Build visuals and layout only; leave clearly
  ...
  her own.
- **Retired: the vertical-center CTA/offer-placement rule.** An earlier
  version of this file required the main point/CTA to sit at or above
  vertical center of the page, measured precisely. She revoked this
  directly — don't measure or enforce it, and don't resurrect it as a
  constraint on future design work.
...
- **Stripe has been removed entirely** (as of the payment-processor-removal
  work) — `worker.js` no longer has `/create-checkout-session`,
  `/record-pass`, `createCheckoutSession`, `recordPass`, `PLAN_CONFIG`, or
  any `STRIPE_SECRET_KEY` reference. There is currently no payment
  processor wired up anywhere in the app — `/report` was never gated on
  payment to begin with, so removing Stripe changed nothing about who can
  generate a reading. `/check-pass` and the `PASSES` KV namespace remain,
  now serving only the family-email bypass below. If/when a payment
  processor is chosen, it still needs to be picked and wired in from
  scratch — this removal did not swap in a replacement.
...
- **Never merge a long-lived feature branch to `main` wholesale without
  ...
  been used across many earlier sessions for the visual entry-page
  redesign (the "glass/sparkle connected-node" work — see the Design-
  process history further up this file) before this session ever used it
  for the content-definitions audit. When the content-audit work was
  merged to `main` via a normal PR, `main` only had 0 commits git
  couldn't fast-forward past, so the merge looked completely clean — but
  it carried the entire unrelated, half-finished visual redesign (white/
  cream backgrounds, black text, ribbon/wisp SVG effects) live along
  with it, replacing the actual approved black site. She caught it
  immediately and it was reverted within the same session, but it should
  never have shipped in the first place. **Before merging any branch to
  `main`, diff the specific files you actually touched against `main`
  directly (`git diff main...HEAD -- <file>`) and confirm every hunk is
  something you recognize and intended** — a clean fast-forward or a
  conflict-free merge is not the same thing as "this branch only
  contains what I meant to ship." After the revert, the branch itself
  was reset to match `main` and the wanted content work (the `ASTRO_DEFS`
  rewrite, the two `worker.js` `REPORT_SYSTEM_PROMPT` additions, and this
  file's own tracking updates) was re-applied on top of that clean base
  by manually extracting just those specific objects/functions — not by
  trusting the branch's history. The old branch state with the visual
  redesign in it no longer exists on `claude/vibrant-glossy-website-
  design-tgzzrd`.
